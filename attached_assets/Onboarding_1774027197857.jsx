  ...
  return (
    <div style={{position:"relative",width:250,height:220,margin:"0 auto"}}>
      ...
      {[
        ...
        <div key={i} style={{position:"absolute",...f.s,background:"white",border:"1px solid var(--border-strong)",
          borderRadius:16,padding:"5px 10px",fontSize:18,animation:`float ${f.dur} ease-in-out infinite`,
          animationDelay:f.delay,boxShadow:"0 4px 16px rgba(0,0,0,0.1)"}}>{f.flag}</div>
      ))}
    </div>
  );
}
...
function DashVisual() {
  return (
    <div style={{width:200,margin:"0 auto",background:"white",borderRadius:20,
      ...
      {[{n:"🇺🇸  +1 (415) 823-4921",a:true},{n:"🇬🇧  +44 7700 123 456",a:false},{n:"🇦🇺  +61 4 1234 5678",a:false}].map((item,i)=>(
        <div key={i} style={{margin:"6px 10px",padding:"9px 10px",background:item.a?"var(--accent-dim)":"var(--bg-raised)",
          borderRadius:9,fontSize:9,fontWeight:600,color:item.a?"var(--accent)":"var(--text-secondary)",
          borderLeft:`2.5px solid ${item.a?"var(--accent)":"transparent"}`}}>{item.n}</div>
      ))}
      <div style={{margin:"8px 10px 10px",padding:"7px 10px",background:"var(--green-dim)",borderRadius:9,
...
function LockVisual() {
  return (
    <div style={{position:"relative",width:220,height:200,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"center"}}>
      ...
      {[{txt:"🔒 Encrypted",c:"var(--accent)",s:{top:20,left:5}},{txt:"✓ Verified",c:"var(--green)",s:{top:60,right:0}},
        {txt:"🚫 No spam",c:"var(--red)",s:{bottom:36,left:10}},{txt:"👁 Private",c:"var(--blue)",s:{bottom:10,right:10}}].map((b,i)=>(
        <div key={i} style={{position:"absolute",...b.s,background:"white",border:"1px solid var(--border-strong)",
          borderRadius:14,padding:"5px 10px",fontSize:9.5,fontWeight:700,color:b.c,
          boxShadow:"0 3px 12px rgba(0,0,0,0.08)",whiteSpace:"nowrap"}}>{b.txt}</div>
      ))}
    </div>
  );
}
...
export default function Onboarding() {
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",
      background:sl.bg,transition:"background 0.6s ease"}}>
      <StatusBar/>
      <div style={{display:"flex",justifyContent:"flex-end",padding:"0 22px 4px"}}>
        {step < SLIDES.length - 1 && (
          <button onClick={() => navigate("auth")} style={{background:"none",border:"none",
            fontSize:13,color:"var(--text-muted)",fontWeight:500}}>Skip</button>
        )}
      </div>
      <div key={`v${step}`} className="fade-up" style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"10px 24px"}}>
        <Visual/>
      </div>
      <div style={{background:"var(--bg-surface)",borderRadius:"28px 28px 0 0",
        padding:"28px 26px 36px",borderTop:"1px solid var(--border)"}}>
        <div key={`c${step}`} className="fade-up">
          <div style={{display:"inline-flex",alignItems:"center",background:"var(--accent-dim)",
            color:"var(--accent)",fontSize:9.5,fontWeight:700,letterSpacing:1.6,
            padding:"4px 11px",borderRadius:99,marginBottom:14}}>{sl.tag}</div>
          <h1 style={{fontSize:30,fontWeight:700,color:"var(--text-primary)",lineHeight:1.18,
            marginBottom:10,letterSpacing:"-0.6px",whiteSpace:"pre-line"}}>{sl.title}</h1>
        ...
        <div style={{display:"flex",gap:5,marginBottom:20,alignItems:"center"}}>
          {SLIDES.map((_,i)=>(
            <div key={i} style={{height:3,borderRadius:99,background:i===step?"var(--accent)":"var(--bg-hover)",
              width:i===step?24:7,transition:"all 0.35s ease"}}/>
          ))}
        </div>
        ...
        {step === 0 && (
          <button onClick={() => navigate("auth")} style={{width:"100%",height:40,background:"none",
            border:"none",color:"var(--text-muted)",fontSize:13,marginTop:4}}>
