  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{background:"var(--bg-surface)",padding:"12px 16px",borderBottom:"1px solid var(--border)",
        ...
        <div style={{width:38,height:38,borderRadius:"var(--r-sm)",background:"var(--accent-dim)",
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{convo.flag}</div>
        <div style={{flex:1}}>
        ...
        <button style={{width:36,height:36,borderRadius:"var(--r-sm)",background:"var(--green-dim)",
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px",display:"flex",flexDirection:"column",gap:8}}>
        ...
        {msgs.map(m=>(
          <div key={m.id} className="fade-up" style={{display:"flex",justifyContent:m.from==="me"?"flex-end":"flex-start"}}>
            <div style={{maxWidth:"76%",padding:"10px 13px",
              ...
              <div style={{fontSize:9.5,marginTop:4,textAlign:"right",
                color:m.from==="me"?"rgba(0,0,0,0.35)":"var(--text-muted)"}}>{m.time}</div>
            </div>
          </div>
        ))}
        <div ref={endRef}/>
      ...
      <div style={{background:"var(--bg-surface)",borderTop:"1px solid var(--border)",
        padding:"10px 13px",display:"flex",gap:9,alignItems:"center",flexShrink:0}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()}
          placeholder="Message…" style={{flex:1,height:42,borderRadius:99,
            border:"1.5px solid var(--border)",background:"var(--bg-input)",
            padding:"0 16px",fontSize:13.5,color:"var(--text-primary)",outline:"none"}}/>
        <button className="press" onClick={send} style={{width:42,height:42,borderRadius:"50%",
          background:"var(--accent)",border:"none",display:"flex",alignItems:"center",
          justifyContent:"center",flexShrink:0,boxShadow:"0 3px 12px var(--accent-glow)",
...
export default function Inbox({ defaultTab = "messages" }) {
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      <div style={{background:"var(--bg-surface)",padding:"10px 20px 0",
        ...
        <div style={{display:"flex"}}>
          {[{id:"messages",l:"Messages"},{id:"calls",l:"Call Log"}].map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{flex:1,height:36,background:"none",border:"none",
              borderBottom:`2px solid ${tab===t.id?"var(--accent)":"transparent"}`,
              color:tab===t.id?"var(--accent)":"var(--text-muted)",
              fontSize:13.5,fontWeight:tab===t.id?600:400,transition:"all 0.15s",cursor:"pointer"}}>{t.l}</button>
          ))}
        </div>
      </div>
      <div style={{flex:1,overflowY:"auto"}}>
        {tab==="messages" ? MOCK_MESSAGES.map(m=>(
          <div key={m.id} onClick={()=>setConvo(m)} className="row-hover"
            style={{display:"flex",alignItems:"center",padding:"13px 18px",gap:11,
              borderBottom:"1px solid var(--border)",background:"var(--bg-base)",
              cursor:"pointer",transition:"background 0.12s"}}>
            <div style={{width:46,height:46,borderRadius:"var(--r-md)",
              background:m.unread>0?"var(--accent-dim)":"var(--bg-raised)",
              display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,
              flexShrink:0,position:"relative"}}>
              {m.flag}
              {m.type==="missed"&&<div style={{position:"absolute",bottom:0,right:0,width:15,height:15,
                borderRadius:"50%",background:"var(--red)",border:"2px solid var(--bg-base)",
                display:"flex",alignItems:"center",justifyContent:"center"}}>
                <Ic d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-.4-8.67A2 2 0 012.18 1h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.91 8.15a16 16 0 006.93 6.93l1.51-1.51a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z"
                  size={7} color="#fff" sw={3}/>
              </div>}
            </div>
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:12.5,color:m.type==="missed"?"var(--red)":"var(--text-secondary)",
                  overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:200,
                  fontWeight:m.type==="missed"?600:400}}>
                  {m.type==="missed"?"Missed call":m.type==="voicemail"?`Voicemail · ${m.preview.split(": ")[1]||""}`:m.preview}
                </span>
                {m.unread>0&&<div style={{minWidth:18,height:18,borderRadius:99,padding:"0 4px",
                  background:"var(--red)",display:"flex",alignItems:"center",justifyContent:"center",
                  fontSize:10,fontWeight:700,color:"#fff",marginLeft:6}}>{m.unread}</div>}
              </div>
            </div>
          </div>
        )) : MOCK_CALLS.map(cl=>{
          ...
          return (
            <div key={cl.id} className="row-hover" style={{display:"flex",alignItems:"center",padding:"13px 18px",
              gap:11,borderBottom:"1px solid var(--border)",background:"var(--bg-base)",
              cursor:"pointer",transition:"background 0.12s"}}>
              <div style={{width:46,height:46,borderRadius:"var(--r-md)",background:cv.bg,
                display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                <Ic d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.81a19.79 19.79 0 01-.4-8.67A2 2 0 012.18 1h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L6.91 8.15a16 16 0 006.93 6.93l1.51-1.51a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z"
                  size={17} color={cv.c} sw={2}/>
              </div>
              <div style={{flex:1}}>
