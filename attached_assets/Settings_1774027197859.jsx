...
// Every row navigates to the correct screen or triggers a sheet.
import React, { useState } from "react";
import { Ic, Toggle, Pill, DeleteSheet } from "../components/UI";
import { useApp } from "../context/AppContext";
import { PLANS } from "../data/mockData";

function SettingsRow({ iconD, label, sublabel, right, danger, onClick, last }) {
  return (
    <div onClick={onClick} className={onClick ? "row-hover" : ""} style={{display:"flex",alignItems:"center",
      gap:12,padding:"14px 16px",borderBottom:last?"none":"1px solid var(--border)",
  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div style={{background:"var(--bg-surface)",borderRadius:"var(--r-lg)",
        border:"1px solid var(--border)",overflow:"hidden"}}>{children}</div>
    </div>
  );
}
...
export default function Settings() {
  ...
  return (
    <div style={{flex:1,overflowY:"auto",background:"var(--bg-base)"}}>
      ...
      <div className="fade-up row-hover" onClick={() => navigate("profile")} style={{background:"var(--bg-surface)",
        borderBottom:"1px solid var(--border)",padding:"16px 20px 20px",
        display:"flex",alignItems:"center",gap:14,marginBottom:20,cursor:"pointer",transition:"background 0.12s"}}>
        <div style={{width:56,height:56,borderRadius:"var(--r-md)",background:"var(--accent)",
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:700,
          color:"var(--bg-base)",boxShadow:"0 4px 14px var(--accent-glow)"}}>V</div>
        <div style={{flex:1}}>
