  ...
  return (
    <div className="fade-up" style={{animationDelay:`${index*0.07}s`,position:"relative"}}>
      <div onClick={() => setOpen(o => !o)} className="row-hover"
        style={{display:"flex",alignItems:"center",gap:13,padding:"13px 20px",cursor:"pointer",
          background:"var(--bg-surface)",transition:"background 0.12s"}}>
        <div style={{width:42,height:42,borderRadius:13,background:"var(--bg-raised)",
      ...
      {open && (
        <div className="fade-up" style={{background:"var(--bg-raised)",borderTop:"1px solid var(--border)",
          borderBottom:"1px solid var(--border)",padding:"10px 20px",display:"flex",gap:8}}>
          <button className="press" style={{flex:1,height:36,background:"var(--accent)",border:"none",
            borderRadius:"var(--r-sm)",color:"var(--bg-base)",fontSize:12,fontWeight:700,
            display:"flex",alignItems:"center",justifyContent:"center",gap:5,
            boxShadow:"0 2px 8px var(--accent-glow)",transition:"transform 0.1s"}}>
          ...
          <button className="press" onClick={() => navigate("inbox")} style={{flex:1,height:36,
            background:"var(--bg-surface)",border:"1px solid var(--border)",borderRadius:"var(--r-sm)",
            color:"var(--text-secondary)",fontSize:12,fontWeight:600,
            display:"flex",alignItems:"center",justifyContent:"center",gap:5,transition:"transform 0.1s"}}>
            <Ic d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" size={12} color="var(--text-secondary)"/> Message
          </button>
          <button className="press" onClick={openDetail} style={{width:36,height:36,
            background:"var(--bg-surface)",border:"1px solid var(--border)",borderRadius:"var(--r-sm)",
          ...
          <button className="press" onClick={() => setShowDelete(true)} style={{width:36,height:36,
            background:"var(--red-dim)",border:"1px solid rgba(220,38,38,0.15)",borderRadius:"var(--r-sm)",
            display:"flex",alignItems:"center",justifyContent:"center",transition:"transform 0.1s"}}>
...
export default function Dashboard() {
  ...
  const planLabels = { basic:"Basic", unlimited:"Unlimited", global:"Global" };
  ...
  return (
    <div style={{flex:1,overflowY:"auto",background:"var(--bg-base)"}}>
      ...
      <div style={{background:"var(--bg-surface)",padding:"6px 20px 20px",borderBottom:"1px solid var(--border)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
          ...
          <div style={{display:"flex",gap:9,alignItems:"center"}}>
            <button style={{width:40,height:40,borderRadius:"var(--r-md)",background:"var(--bg-raised)",
              ...
              <Ic d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0"
                size={17} color="var(--text-secondary)"/>
            ...
            <button onClick={() => navigate("profile")} style={{width:40,height:40,borderRadius:"var(--r-md)",
              ...
              fontSize:15,fontWeight:700,color:"var(--bg-base)",border:"none",cursor:"pointer"}}>V</button>
          </div>
        </div>
        {/* Stats strip */}
        <div style={{display:"flex",gap:8}}>
          {[
            ...
            <div key={i} className="fade-up" style={{flex:1,background:"var(--bg-raised)",borderRadius:"var(--r-md)",
              padding:"12px 10px",border:"1px solid var(--border)",animationDelay:`${i*0.06}s`}}>
      ...
      {/* Plan banner */}
      <div onClick={() => navigate("paywall")} style={{margin:"10px 16px 0",background:"var(--accent-dim)",
      ...
      <div style={{background:"var(--bg-surface)",marginTop:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px 10px"}}>
          ...
          <button onClick={() => navigate("picker")} style={{fontSize:12,color:"var(--accent)",fontWeight:600,
            background:"none",border:"none",display:"flex",alignItems:"center",gap:3,cursor:"pointer"}}>
        ...
        {numbers.length === 0 ? (
          <div style={{padding:"30px 20px",textAlign:"center"}}>
            ...
            <button className="press" onClick={() => navigate("picker")} style={{padding:"10px 24px",
              background:"var(--accent)",border:"none",borderRadius:"var(--r-md)",
              color:"var(--bg-base)",fontSize:13,fontWeight:700,boxShadow:"0 4px 14px var(--accent-glow)",transition:"transform 0.1s"}}>
              Get a Number →
            </button>
          </div>
        ) : (
      ...
      <div style={{padding:"10px 16px 28px"}}>
        <div className="press fade-up" onClick={() => navigate("picker")} style={{background:"var(--bg-surface)",
          border:"1px solid var(--border-strong)",borderRadius:"var(--r-lg)",padding:"16px 18px",
          marginTop:10,cursor:"pointer",animationDelay:"0.25s",position:"relative",overflow:"hidden",
          transition:"transform 0.1s",display:"flex",alignItems:"center",gap:14}}>
          <div style={{position:"absolute",top:-20,right:-20,width:80,height:80,borderRadius:"50%",
            background:"var(--accent-glow)",filter:"blur(24px)",pointerEvents:"none"}}/>
