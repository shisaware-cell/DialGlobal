...
export default function Picker() {
  ...
  // On Basic, only US/UK/CA are allowed
  ...
  const provision = () => {
    ...
    setLoading(true);
    // TODO: call provisionNumber(number, type) from api.js
    setTimeout(() => {
  ...
  if (done) return (
    <div className="scale-in" style={{flex:1,display:"flex",flexDirection:"column",
      alignItems:"center",justifyContent:"center",background:"var(--bg-base)",padding:"0 32px"}}>
      <div style={{width:72,height:72,borderRadius:"50%",background:"var(--green-dim)",
        border:"1px solid rgba(22,163,74,0.2)",display:"flex",alignItems:"center",
        justifyContent:"center",marginBottom:22,animation:"pop 0.5s ease"}}>
        <Ic d="M20 6L9 17l-5-5" size={30} color="var(--green)" sw={2.5}/>
      </div>
      <h2 style={{fontSize:24,fontWeight:700,color:"var(--text-primary)",textAlign:"center",marginBottom:8,letterSpacing:"-0.5px"}}>
        Number Activated 🎉
      </h2>
      <p style={{fontSize:13,color:"var(--text-secondary)",textAlign:"center",marginBottom:28,lineHeight:1.7}}>
        Your new {selected.name} number is live and ready.
      </p>
      <div style={{background:"var(--bg-surface)",border:"1px solid var(--border-strong)",
        borderRadius:"var(--r-lg)",padding:"20px 24px",marginBottom:28,textAlign:"center",width:"100%"}}>
      ...
      <button className="press" onClick={() => { setDone(false); setSelected(null); setSearch(""); navigate("home"); }}
        style={{width:"100%",height:52,background:"var(--accent)",border:"none",borderRadius:"var(--r-md)",
          color:"var(--bg-base)",fontSize:15,fontWeight:700,boxShadow:"0 6px 20px var(--accent-glow)",
          transition:"transform 0.1s"}}>Done</button>
    </div>
  );
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:"var(--bg-base)",position:"relative"}}>
      <div style={{background:"var(--bg-surface)",padding:"8px 20px 0",borderBottom:"1px solid var(--border)",flexShrink:0}}>
        ...
        {/* Plan limit notice */}
        {atLimit && (
          <div onClick={() => navigate("paywall")} style={{margin:"10px 0 8px",background:"var(--accent-dim)",
            border:"1px solid rgba(232,160,32,0.25)",borderRadius:"var(--r-sm)",padding:"8px 12px",
            display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}>
            <span style={{fontSize:12}}>⚠️</span>
            <span style={{fontSize:12,color:"var(--accent)",fontWeight:600}}>
              You've reached your {plan?.name} limit ({plan?.numberLimit} numbers). Upgrade →
            </span>
          </div>
        )}
        ...
        <div style={{position:"relative",margin:"10px 0 11px"}}>
          ...
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search country or code…"
            style={{width:"100%",height:42,borderRadius:"var(--r-md)",border:"1.5px solid var(--border)",
              background:"var(--bg-input)",paddingLeft:36,paddingRight:14,fontSize:13.5,
        ...
        <div style={{display:"flex",gap:7,paddingBottom:13,overflowX:"auto"}}>
          {[{id:"all",l:"All"},{id:"popular",l:"⭐ Popular"},{id:"instant",l:"⚡ Instant"}].map(f=>(
            <button key={f.id} onClick={()=>setFilter(f.id)} style={{flexShrink:0,height:28,padding:"0 13px",
              borderRadius:99,border:`1.5px solid ${filter===f.id?"var(--accent)":"var(--border)"}`,
          ...
          return (
            <div key={c.code} onClick={()=>setSelected(c)} className="fade-up" style={{
              ...
              <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                ...
                <div style={{fontSize:9,fontWeight:700,padding:"2px 7px",borderRadius:99,
                  background:c.instant?"var(--green-dim)":"var(--bg-raised)",
                  color:c.instant?"var(--green)":"var(--text-muted)"}}>
                  {c.instant?"⚡ Instant":"📋 Docs req."}
                </div>
              </div>
              {selected?.code===c.code && (
                <div style={{width:18,height:18,borderRadius:"50%",background:"var(--accent)",
                  display:"flex",alignItems:"center",justifyContent:"center",marginLeft:10,animation:"pop 0.2s ease"}}>
        ...
        <div className="slide-up" style={{background:"var(--bg-surface)",borderTop:"1px solid var(--border)",
          ...
          <div style={{display:"flex",gap:7,marginBottom:12}}>
            {["permanent","temporary"].map(t=>(
              <button key={t} onClick={()=>setType(t)} style={{flex:1,height:37,borderRadius:"var(--r-sm)",
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
            marginBottom:12,padding:"10px 12px",background:"var(--bg-raised)",borderRadius:"var(--r-sm)"}}>
          ...
          <button className="press" onClick={provision} disabled={loading} style={{width:"100%",height:52,
            ...
            boxShadow:loading?"none":"0 6px 20px var(--accent-glow)",transition:"transform 0.1s"}}>
