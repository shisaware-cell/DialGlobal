    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"var(--bg-base)",overflow:"hidden"}}>
      <div style={{background:"var(--bg-surface)",padding:"10px 18px 0",borderBottom:"1px solid var(--border)",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
          ...
          <button onClick={() => editing ? save() : setEditing(true)} style={{fontSize:12,fontWeight:600,
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 16px 28px"}}>
        <div className="fade-up" style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"24px 0 20px"}}>
          <div style={{width:80,height:80,borderRadius:24,background:"var(--accent)",display:"flex",
            alignItems:"center",justifyContent:"center",fontSize:28,fontWeight:700,color:"var(--bg-base)",
            boxShadow:"0 8px 28px var(--accent-glow)",marginBottom:14}}>V</div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:20}}>
          {[
            ...
            <div key={i} className="fade-up" style={{background:"var(--bg-surface)",border:"1px solid var(--border)",
              borderRadius:"var(--r-md)",padding:"13px 12px",animationDelay:`${i*0.06}s`}}>
        ...
        <div style={{background:"var(--bg-surface)",border:"1px solid var(--border)",
          borderRadius:"var(--r-lg)",overflow:"hidden",marginBottom:14}}>
          ...
          {[{label:"Full Name",value:name,set:setName},{label:"Email",value:email,set:setEmail}].map((f,i)=>(
            <div key={i} style={{padding:"12px 16px",borderBottom:i===0?"1px solid var(--border)":"none"}}>
              <div style={{fontSize:10.5,fontWeight:600,color:"var(--text-muted)",marginBottom:5,letterSpacing:0.4}}>
                {f.label.toUpperCase()}
              </div>
              {editing ? (
                <input value={f.value} onChange={e=>f.set(e.target.value)} style={{width:"100%",
                  background:"var(--bg-input)",border:"1.5px solid var(--border)",borderRadius:"var(--r-sm)",
                  padding:"8px 12px",fontSize:13.5,color:"var(--text-primary)",outline:"none"}}/>
              ) : (
                <div style={{fontSize:13.5,color:"var(--text-primary)",
                  fontFamily:i===1?"var(--font-mono)":"inherit"}}>{f.value}</div>
              )}
            </div>
          ))}
        </div>
        ...
        {saved && (
          <div className="fade-up" style={{background:"var(--green-dim)",border:"1px solid rgba(22,163,74,0.2)",
            borderRadius:"var(--r-md)",padding:"11px 16px",marginBottom:14,
        ...
        <div style={{background:"var(--bg-surface)",border:"1px solid var(--border)",
          ...
          {currentPlan !== "global" && (
            <div style={{borderTop:"1px solid var(--border)",padding:"10px 16px"}}>
              <button onClick={() => navigate("paywall")} style={{fontSize:12,fontWeight:600,color:"var(--accent)",
                background:"none",border:"none",display:"flex",alignItems:"center",gap:4,cursor:"pointer"}}>
        ...
        <div style={{background:"var(--bg-surface)",border:"1px solid var(--border)",
          ...
          <button onClick={() => setShowSignOut(true)} style={{width:"100%",padding:"14px 16px",background:"none",
            border:"none",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
            <div style={{width:34,height:34,borderRadius:"var(--r-sm)",background:"var(--red-dim)",
