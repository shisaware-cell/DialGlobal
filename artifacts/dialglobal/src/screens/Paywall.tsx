...
export default function Paywall({ onClose }: { onClose?: () => void }) {
  const { goBack, upgradePlan, currentPlan, closePaywall } = useApp();
  ...
  const PLAN_META: Record<string, { tag: string | null; tagColor: string | null }> = {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "var(--bg-base)", overflow: "hidden" }}>
      <div style={{ background: "var(--bg-surface)", padding: "10px 20px 14px", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
          <button onClick={handleBack} style={{ background: "none", border: "none", display: "flex", padding: 4, cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", background: "var(--bg-raised)", borderRadius: "var(--r-md)", padding: 3, marginTop: 12 }}>
          {["monthly", "yearly"].map(b => (
            <button key={b} onClick={() => setBilling(b)} style={{
              flex: 1, height: 33, borderRadius: 11, border: "none",
              ...
              boxShadow: billing === b ? "0 1px 4px rgba(0,0,0,0.08)" : "none", cursor: "pointer"
            }}>
        ...
        {PLANS.map((plan, i) => {
          ...
          return (
            <div key={plan.id} onClick={() => setSelected(plan.id)} className="fade-up"
              style={{
                animationDelay: `${i * 0.06}s`, borderBottom: "1px solid var(--border)",
              ...
              {meta.tag && (
                <div style={{ height: 22, background: meta.tagColor!, display: "flex", alignItems: "center", paddingLeft: 16 }}>
                  <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 1.5, color: plan.id === "global" ? "#fff" : "var(--bg-base)" }}>{meta.tag}</span>
                </div>
              )}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0, marginLeft: 12 }}>
                      <div style={{ fontSize: 20, fontWeight: 700, fontFamily: "var(--font-mono)", letterSpacing: "-0.4px", color: isSel ? "var(--accent)" : "var(--text-primary)" }}>${dp}</div>
        ...
        <div style={{ margin: "0 16px 20px", background: "var(--bg-raised)", borderRadius: "var(--r-md)", padding: "12px 14px" }}>
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 0", borderBottom: i < 3 ? "1px solid var(--border)" : "none" }}>
              <span style={{ fontSize: 12, fontWeight: (c as any).us ? 700 : 400, color: (c as any).us ? "var(--text-primary)" : "var(--text-secondary)" }}>{c.name}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{c.note}</span>
                <span style={{ fontSize: 12, fontWeight: 700, fontFamily: "var(--font-mono)", color: (c as any).us ? "var(--accent)" : "var(--text-secondary)" }}>{c.price}/mo</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ background: "var(--bg-surface)", borderTop: "1px solid var(--border)", padding: "12px 18px 20px", flexShrink: 0 }}>
        ...
        <button className="press" onClick={handleSubscribe} disabled={loading || currentPlan === selected} style={{
          ...
          boxShadow: currentPlan === selected ? "none" : "0 6px 22px var(--accent-glow)",
          transition: "transform 0.1s", cursor: currentPlan === selected ? "default" : "pointer"
        }}>
