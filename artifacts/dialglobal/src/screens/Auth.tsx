...
export default function Auth() {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "var(--bg-base)", overflow: "hidden" }}>
      <div style={{
        height: 180, background: "radial-gradient(ellipse at 50% 0%, #FDE9C0 0%, var(--bg-base) 80%)",
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flexShrink: 0
      }}>
        <div style={{
          width: 56, height: 56, borderRadius: 16, background: "var(--accent)", display: "flex",
      ...
      <div style={{
        flex: 1, background: "var(--bg-surface)", borderRadius: "28px 28px 0 0",
        ...
        <div style={{ display: "flex", background: "var(--bg-raised)", borderRadius: "var(--r-md)", padding: 3, marginBottom: 22 }}>
          {(["login", "signup"] as const).map(m => (
            <button key={m} onClick={() => { setMode(m); setErr(""); }} style={{
              flex: 1, height: 36, borderRadius: 11, border: "none",
              background: mode === m ? "var(--bg-surface)" : "transparent",
              color: mode === m ? "var(--text-primary)" : "var(--text-muted)",
              fontSize: 13, fontWeight: mode === m ? 600 : 400, transition: "all 0.2s",
              boxShadow: mode === m ? "0 1px 4px rgba(0,0,0,0.08)" : "none", cursor: "pointer"
            }}>
        ...
        {mode === "login" && (
          <div style={{ textAlign: "right", marginTop: 8 }}>
            <button style={{ fontSize: 12, color: "var(--accent)", background: "none", border: "none", fontWeight: 500, cursor: "pointer" }}>Forgot password?</button>
          </div>
        )}

        <button className="press" onClick={submit} disabled={loading} style={{
          width: "100%", height: 52, background: "var(--accent)", border: "none", borderRadius: "var(--r-md)",
          color: "var(--bg-base)", fontSize: 15, fontWeight: 700, marginTop: 22,
          display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
          boxShadow: "0 6px 22px var(--accent-glow)", transition: "transform 0.1s", cursor: "pointer"
        }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {["Google", "Apple"].map(s => (
            <button key={s} className="press" style={{
              flex: 1, height: 46, background: "var(--bg-raised)", border: "1px solid var(--border)",
